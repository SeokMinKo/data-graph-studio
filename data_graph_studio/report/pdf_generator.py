...
class PDFReportGenerator(ReportGenerator):
    ...
    def _generate_with_fpdf(
        ...
        pdf.set_margins(
            margins.get("left", 20), margins.get("top", 20), margins.get("right", 20)
        )
    ...
    def _add_fpdf_header(
        ...
        pdf.set_xy(10, 45)
        meta_text = (
            f"Generated: {report_data.metadata.created_at.strftime('%Y-%m-%d %H:%M')}"
        )
    ...
    def _add_fpdf_section_title(self, pdf: "FPDF", title: str, primary_color: tuple):
        """섹션 제목 추가"""
        pdf.ln(10)
            ...
            if chart.image_bytes:
                try:
                    ...
                    with tempfile.NamedTemporaryFile(
                        suffix=f".{chart.image_format}", delete=False
                    ) as tmp:
    ...
    def _add_fpdf_comparison(
        ...
        if report_data.comparisons:
            ...
            for comp in report_data.comparisons:
                ...
                pdf.cell(
                    0,
                    5,
                    f"t-statistic: {comp.test_statistic:.4f}, p-value: {comp.p_value:.4f}",
                    ln=True,
                )
        ...
        for diff in report_data.differences:
            ...
            pdf.cell(
                0,
                5,
                f"Positive (A > B): {diff.positive_count} ({diff.positive_percentage:.1f}%)",
                ln=True,
            )
            pdf.cell(10)
            pdf.cell(
                0,
                5,
                f"Negative (A < B): {diff.negative_count} ({diff.negative_percentage:.1f}%)",
                ln=True,
            )
            pdf.cell(10)
            pdf.cell(
                0,
                5,
                f"No Change: {diff.neutral_count} ({diff.neutral_percentage:.1f}%)",
                ln=True,
            )
            pdf.ln(3)
        ...
        for table in report_data.tables:
            ...
            if table.total_rows > table.shown_rows:
                ...
                pdf.cell(
                    0,
                    5,
                    f"Showing {min(15, table.shown_rows)} of {table.total_rows} rows",
                    ln=True,
                )
