...
Examples:
  ...
  dgs server --port 8080
    ...
    plot_parser.add_argument(
        "input", help="Input data file (CSV, Excel, Parquet, JSON)"
    )
    plot_parser.add_argument("-x", "--x-column", help="X-axis column name")
    plot_parser.add_argument(
        "-y", "--y-columns", help="Y-axis column names (comma-separated)"
    )
    plot_parser.add_argument(
        "-c",
        "--chart",
    ...
    watch_parser = subparsers.add_parser(
        "watch", help="Watch file and auto-update chart"
    )
    ...
    watch_parser.add_argument(
        "--interval", type=float, default=5, help="Check interval in seconds"
    )
...
def cmd_plot(args):
    ...
    try:
        ...
        return _plot_with_matplotlib(
            df, x_col, y_cols, chart_type, title, output_file, args
        )
...
def _plot_with_matplotlib(df, x_col, y_cols, chart_type, title, output_file, args):
    ...
    try:
        ...
        fig, ax = plt.subplots(
            figsize=(args.width / 100, args.height / 100), dpi=args.dpi
        )
