...
class DataSampler:
    ...
    def lttb(
        ...
        for i in range(1, threshold - 1):
            ...
            for j in range(bucket_start, min(bucket_end, n)):
                # 삼각형 면적 계산
                area = abs(
                    (x[a] - avg_x) * (y[j] - y[a]) - (x[a] - x[j]) * (avg_y - y[a])
                )
    ...
    def min_max_per_bucket(
        x: np.ndarray, y: np.ndarray, n_buckets: int
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
        버킷별 Min/Max 샘플링
    ...
    @staticmethod
    def random_sample(
        x: np.ndarray, y: np.ndarray, n_samples: int, seed: int = 42
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
        무작위 샘플링
    ...
    def stratified_sample(
        x: np.ndarray, y: np.ndarray, groups: np.ndarray, n_samples: int, seed: int = 42
