...
class Parser:
    ...
    def parse_function_call(self, name: str) -> Dict:
        ...
        if self.current.type != TokenType.RPAREN:
            raise ExpressionError(
                f"Expected ')' in function call at position {self.current.position}"
            )
        self.advance()
...
class ExpressionEngine:
    ...
    def _evaluate_function(self, node: Dict, df: pl.DataFrame) -> pl.Series:
        ...
        if func_name == "IF":
            ...
            tmp = pl.DataFrame(
                {"__cond__": condition, "__then__": then_value, "__else__": else_value}
            )
    ...
    def add_column(
        self, df: pl.DataFrame, column_name: str, expression: str
    ) -> pl.DataFrame:
        """
        계산 필드 컬럼 추가
    ...
    def validate(self, expression: str, df: pl.DataFrame) -> Tuple[bool, Optional[str]]:
        """
        수식 유효성 검사
