...
class DataGraphStudio:
    ...
    def plot(
        self, x: str = None, y: Union[str, List[str]] = None, chart: str = None
    ) -> "DataGraphStudio":
        """
        그래프 설정
        ...
        if y:
            ...
            self._y_columns = (
                numeric_cols[:3] if numeric_cols else self._df.columns[1:2]
            )
    ...
    def set_legend(
        self, show: bool = True, position: str = "right"
    ) -> "DataGraphStudio":
        """범례 설정"""
    ...
    def from_config(config: Dict[str, Any]) -> "DataGraphStudio":
        ...
        # 플롯 설정
        dgs.plot(
            x=config.get("x"), y=config.get("y"), chart=config.get("chart", "line")
        )
    ...
    @staticmethod
    def quick_plot(
        data: Union[str, Dict, pl.DataFrame],
        x: str = None,
        y: Union[str, List[str]] = None,
        chart: str = "line",
        output: str = None,
        show: bool = False,
    ) -> Optional["DataGraphStudio"]:
        """
        빠른 플롯 (한 줄)
    ...
    def _render_and_save(self, df: pl.DataFrame, path: str, dpi: int):
        ...
        x_data = (
            df[self._x_column].to_list() if self._x_column else list(range(len(df)))
        )
        ...
        for i, y_col in enumerate(self._y_columns):
            ...
            if self._chart_type == "line":
                ax.plot(
                    x_data, y_data, label=y_col, color=color, marker="o", markersize=2
                )
                ...
                ax.fill_between(
                    range(len(x_data)), y_data, alpha=0.5, label=y_col, color=color
                )
