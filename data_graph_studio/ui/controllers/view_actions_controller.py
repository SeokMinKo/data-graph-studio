...
class ViewActionsController:
    ...
    def _on_theme_changed(self, theme_id: str):
        ...
        for attr in (
            "_overlay_stats_widget",
            "_comparison_stats_panel",
            "_compare_toolbar",
            "_profile_overlay_renderer",
    ...
    def _on_toggle_statistics_overlay(self, checked: bool = None):
        """통계 오버레이 표시 토글"""
        if checked is None:
            checked = self.w._graph_element_actions.get(
                "statistics_overlay", QAction()
            ).isChecked()
        ...
        self.w.statusbar.showMessage(
            f"Statistics overlay {'shown' if checked else 'hidden'}", 2000
        )
    ...
    def _on_toggle_axis_labels(self, checked: bool = None):
        ...
        self.w.statusbar.showMessage(
            f"Axis labels {'shown' if checked else 'hidden'}", 2000
        )
    ...
    def _on_draw_color_pick(self):
        ...
        color = QColorDialog.getColor(self.w._draw_color, self.w, "Draw Color")
        if color.isValid():
    ...
    def _on_toggle_row_numbers(self, checked: bool = None):
        """행 번호 표시 토글"""
        if checked is None:
            checked = self.w._table_element_actions.get(
                "row_numbers", QAction()
            ).isChecked()
        ...
        self.w.statusbar.showMessage(
            f"Row numbers {'shown' if checked else 'hidden'}", 2000
        )
    ...
    def _on_toggle_column_headers(self, checked: bool = None):
        ...
        self.w.statusbar.showMessage(
            f"Column headers {'shown' if checked else 'hidden'}", 2000
        )
    ...
    def _on_toggle_filter_bar(self, checked: bool = None):
        ...
        self.w.statusbar.showMessage(
            f"Filter bar {'shown' if checked else 'hidden'}", 2000
        )
    ...
    def _on_multi_grid_view(self):
        ...
        layout, ok = QInputDialog.getItem(
            self.w, "Multi-Grid View", "Select grid layout:", layouts, 2, False
        )
        if ok:
            if layout == "Custom...":
                # 커스텀 그리드 설정
                rows, rows_ok = QInputDialog.getInt(
                    self.w, "Custom Grid", "Number of rows:", 2, 1, 10
                )
                if rows_ok:
                    cols, cols_ok = QInputDialog.getInt(
                        self.w, "Custom Grid", "Number of columns:", 2, 1, 10
                    )
                    if cols_ok:
                        self.w.statusbar.showMessage(
                            f"Multi-grid view: {rows}x{cols}", 3000
                        )
            else:
    ...
    def _activate_dashboard_mode(self):
        ...
        if self.w._dashboard_panel is None:
            ...
            self.w._dashboard_panel.exit_requested.connect(
                self.w._deactivate_dashboard_mode
            )
            self.w._dashboard_panel.cell_clicked.connect(
                self.w._on_dashboard_cell_clicked
            )
            self.w._dashboard_panel.preset_changed.connect(
                self.w._on_dashboard_preset_changed
            )
            self.w._dashboard_panel.cell_swap_requested.connect(
                self._on_dashboard_cell_swap
            )
            self.w._dashboard_panel.grid_size_changed.connect(
                self._on_dashboard_grid_size_changed
            )
            self.w._dashboard_panel.name_changed.connect(
                self._on_dashboard_name_changed
            )
    ...
    def _on_dashboard_cell_clicked(self, row: int, col: int):
        ...
        if not profiles:
            QMessageBox.information(
                self.w, "Dashboard", "No profiles available. Create a profile first."
            )
            return
    ...
    def _on_dashboard_grid_size_changed(self, rows: int, cols: int):
        ...
        if before is not None:
            for cell in before.cells:
                if (
                    cell.row + cell.row_span <= rows
                    and cell.col + cell.col_span <= cols
                ):
    ...
    def _on_toggle_annotation_panel(self):
        ...
        if self.w._annotation_panel is None:
            ...
            # Connect signals
            self.w._annotation_panel.navigate_requested.connect(
                self.w._on_annotation_navigate
            )
            self.w._annotation_panel.edit_requested.connect(self.w._on_annotation_edit)
            self.w._annotation_panel.delete_requested.connect(
                self.w._on_annotation_delete
            )
    ...
    def _on_add_annotation(self):
        ...
        if not self.w.state.value_columns and not self.w.state.x_column:
            self.w.statusbar.showMessage(
                "⚠ Display a graph first before adding annotations", 3000
            )
            return
        ...
        if ok and text:
            ...
            ann = Annotation(
                ...
                profile_id=self.w.profile_controller.active_profile_id
                if hasattr(self.w.profile_controller, "active_profile_id")
                else "",
            )
            try:
                ...
                if (
                    self.w._annotation_panel is None
                    or not self.w._annotation_panel.isVisible()
                ):
