...
Extracted from ThemeManager.generate_stylesheet() in theme.py.
Coordinator: calls widget_stylesheet() for Qt built-in widgets, then
appends app-specific panel CSS inline (extracted to _theme_stylesheet_panels.py
in Task 4).
"""
...
def base_stylesheet(t: "Theme") -> str:
    """Return CSS shared by both light and dark themes.

    Args:
        t: Theme instance carrying color tokens and is_light() logic.

    Returns:
        QSS string for all widget styles.
    """
    from ._theme_stylesheet_widgets import widget_stylesheet
    return widget_stylesheet(t) + f"""/* ============ Panel Backgrounds ============ */
            #GraphOptionsPanel {{
