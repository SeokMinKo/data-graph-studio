...
class GridPosition:
    ...
    def overlaps(self, other: 'GridPosition') -> bool:
        """Return True if this grid position overlaps with another."""
    ...
    def to_dict(self) -> Dict:
        """Serialize this grid position to a dictionary."""
        return {'row': self.row, 'col': self.col,
                'row_span': self.row_span, 'col_span': self.col_span}

    @classmethod
    def from_dict(cls, data: Dict) -> 'GridPosition':
        """Deserialize a GridPosition from a dictionary produced by to_dict."""
...
class DashboardItem:
    ...
    def to_dict(self) -> Dict:
        """Serialize this dashboard item to a dictionary."""
    ...
    def from_dict(cls, data: Dict) -> 'DashboardItem':
        """Deserialize a DashboardItem from a dictionary produced by to_dict."""
...
class DashboardLayout:
    ...
    def add_item(self, item: DashboardItem):
        """Add a dashboard item to the layout."""
        self.items.append(item)

    def remove_item(self, item_id: str):
        """Remove the item with the given ID from the layout."""
    ...
    def check_collision(self, position: GridPosition, exclude_id: Optional[str] = None) -> bool:
        """Return True if position overlaps with any existing item (excluding exclude_id)."""
        for item in self.items:
    ...
    def find_empty_position(self, row_span: int = 1, col_span: int = 1) -> Optional[GridPosition]:
        """Return the first grid position that fits the given span without collision, or None."""
    ...
    def to_dict(self) -> Dict:
        """Serialize this dashboard layout to a dictionary."""
...
class Dashboard:
    ...
    def create(cls, name: str, rows: int = 2, cols: int = 2) -> 'Dashboard':
        """Create a new Dashboard with a fresh layout and a generated UUID."""
...
class DashboardManager:
    ...
    def current(self) -> Optional[Dashboard]:
        """Return the currently active Dashboard, or None."""
        if self._current_id:
            return self._dashboards.get(self._current_id)
        return None

    def create_dashboard(self, name: str, rows: int = 2, cols: int = 2) -> Dashboard:
        """Create and register a new dashboard, setting it as current if none exists."""
    ...
    def delete_dashboard(self, dashboard_id: str):
        """Delete a dashboard by ID and update the current pointer if necessary."""
    ...
    def list_dashboards(self) -> List[Dashboard]:
        """Return a list of all registered dashboards."""
        return list(self._dashboards.values())

    def switch_to(self, dashboard_id: str):
        """Set the dashboard with the given ID as the active current dashboard."""
        if dashboard_id in self._dashboards:
