...
class GraphOptionsPanel(QFrame):
    ...
    def _create_axes_tab(self) -> QWidget:
        ...
        self.sliding_window_check.setToolTip(
            "Enable navigation minimap for large datasets"
        )
        ...
        self.x_sliding_window_check.setEnabled(
            True
        )  # Enabled by default since master is checked
        ...
        self.y_sliding_window_check.setEnabled(
            True
        )  # Enabled by default since master is checked
    ...
    def _create_chart_tab(self) -> QWidget:
        ...
        self._combo_series_widget.setSizePolicy(
        ...
        self.grid_split_combo.setToolTip(
            "Column to split data by (uses Filter panel selections)"
        )
        ...
        self.grid_direction_combo.setToolTip(
            "Layout direction: Row=horizontal, Column=vertical, Wrap=auto"
        )
        self.grid_direction_combo.currentIndexChanged.connect(
            self._on_grid_direction_changed
        )
        ...
        self.show_all_data_check.setToolTip(
            "Disable downsampling — render every data point"
        )
    ...
    def _show_chart_recommendations(self):
        ...
        if engine is None or engine.df is None:
            QMessageBox.information(
                self, "Chart Recommendation", "데이터를 먼저 로드하세요."
            )
            return

        x_col = self.state.x_column
        y_cols = (
            [vc.name for vc in self.state.value_columns]
        ...
        if not y_cols:
            QMessageBox.information(
                self, "Chart Recommendation", "Y 컬럼을 선택하세요."
            )
            return
        ...
        if not recs:
            QMessageBox.information(
                self, "Chart Recommendation", "추천할 차트 타입이 없습니다."
            )
            return
        ...
        for chart_type, reason in recs:
            ...
            act.triggered.connect(
                lambda checked=False, ct=chart_type: self._apply_recommended_chart(ct)
            )
        menu.exec(
            self._recommend_btn.mapToGlobal(self._recommend_btn.rect().bottomLeft())
        )
    ...
    def get_grid_view_settings(self) -> Dict[str, Any]:
        ...
        direction_map = {
            0: GridDirection.WRAP,
            1: GridDirection.ROW,
            2: GridDirection.COLUMN,
        }
        return {
            "enabled": self.grid_view_check.isChecked(),
            "split_by": self.grid_split_combo.currentText() or None,
            "direction": direction_map.get(
                self.grid_direction_combo.currentIndex(), GridDirection.WRAP
            ),
