...
class GroupNode:
    ...
    def is_group(self) -> bool:
        ...
        return (
            len(self.children) > 0 or len(self.rows) > 1 or "_count" in self.aggregates
        )
...
class GroupedTableModel(QAbstractItemModel):
    ...
    def __init__(self, parent=None):
        ...
        self._flat_view: List[
            Tuple[GroupNode, Optional[int]]
        ] = []  # (node, row_idx or None for header)
    ...
    def _build_tree_optimized(self):
        ...
        try:
            grouped = (
                self._df.group_by(self._group_columns)
                .agg(agg_exprs)
                .sort(self._group_columns)
            )
        except Exception as e:
            # 실패 시 기본 방식
            logger.debug(
                "Polars groupby failed, falling back to recursive build: %s", e
            )
    ...
    def _build_group_recursive(
        ...
        for value in unique_values:
            ...
            child = GroupNode(
                key=child_key, display_name=display, parent=parent, level=level
            )
            parent.children.append(child)
    ...
    def toggle_expand(self, row: int):
        ...
        if (
            not node.children
            and len(node.rows) <= 1
            and "_count" not in node.aggregates
        ):
    ...
    def _collect_leaf_groups(
        self, node: GroupNode, result: List[Tuple[str, List[int], str]]
    ):
        ...
        if node.children:
            ...
            result.append((node.display_name, node.rows, self._group_colors[color_idx]))
