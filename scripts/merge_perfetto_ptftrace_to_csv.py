#!/usr/bin/env python3
"""Merge multiple Perfetto trace files into one CSV via trace_processor_shell.

Example:
    python scripts/merge_perfetto_ptftrace_to_csv.py \
        traces/*.pftrace \
        --output merged.csv

Notes:
- Accepts *.pftrace, *.perfetto-trace, and *.ptftrace inputs.
- Adds `source_trace` and `source_basename` columns so merged rows stay traceable.
- Uses the same default ftrace_event query shape as DGS' Perfetto pipeline.
"""
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "inputs",
        nargs="*",
        help="Trace files or glob patterns (*.ptftrace, *.pftrace, *.perfetto-trace).",
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
        action="append",
        default=[],
        help="Directory to scan for supported trace files. Can be used multiple times.",
    )
    parser.add_argument(
        "--pattern",
        action="append",
        default=[],
        help="Additional glob pattern(s) to scan inside --input-dir.",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    parser.add_argument(
        "--query-file",
        type=Path,
        help="Optional SQL file. Defaults to the DGS ftrace_event export query.",
    )
    parser.add_argument(
        "--strict",
        action="store_true",
...
def resolve_trace_processor(explicit: str | None) -> str:
    ...
    raise FileNotFoundError(
        "trace_processor_shell not found in PATH. Pass --trace-processor explicitly."
    )
...
def load_query(query_file: Path | None) -> str:
    ...
    return query_file.expanduser().read_text(encoding="utf-8").strip()
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    for trace_path in trace_paths:
        ...
        except Exception as exc:  # pragma: no cover - exercised through CLI path
