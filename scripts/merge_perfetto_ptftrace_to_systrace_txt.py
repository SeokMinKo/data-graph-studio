#!/usr/bin/env python3
"""One-shot pipeline: multiple Perfetto traces -> merged CSV -> systrace txt."""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Directory to scan for trace files. Can be passed multiple times.",
    )
    parser.add_argument(
        "--pattern",
        action="append",
    ...
    parser.add_argument(
        "--no-header",
        action="store_true",
        help="Do not write systrace header comments into txt output.",
    )
