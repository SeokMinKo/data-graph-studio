...
This is intended for the CSV produced by
`scripts/merge_perfetto_ptftrace_to_csv.py`, but it also works with a single
Perfetto-export CSV that contains at least:
    ts,cpu,name,task,pid,details

Example:
    python scripts/convert_merged_perfetto_csv_to_systrace_txt.py \
        merged.csv \
        --output merged_systrace.txt
"""
...
def coerce_timestamp(value: str) -> float:
    try:
        raw = float(value)
    except Exception:
        return 0.0
...
def make_systrace_line(row: dict[str, str]) -> str:
    task = (row.get("task") or "<unknown>").strip() or "<unknown>"
    pid = row.get("pid") or "-1"
    cpu = row.get("cpu") or "0"
    flags = (row.get("flags") or "....").strip() or "...."
    event_raw = row.get("event") or row.get("name") or ""
    event = normalize_event_name(event_raw)
    details = coerce_perfetto_details(event_raw, row.get("details") or "")
    timestamp_value = row.get("timestamp") or row.get("ts") or "0"
    timestamp = coerce_timestamp(timestamp_value)
    return f"{task}-{pid} [{int(cpu):03d}] {flags} {timestamp:.6f}: {event}: {details}".rstrip()


def convert_csv_to_systrace(input_csv: Path, output_txt: Path, include_source_comments: bool) -> int:
    output_txt.parent.mkdir(parents=True, exist_ok=True)
    count = 0
    current_source = None

    with input_csv.open("r", newline="", encoding="utf-8") as src, output_txt.open(
        "w", encoding="utf-8"
    ) as dst:
        ...
        for row in reader:
            ...
            dst.write(make_systrace_line(row) + "\n")
            count += 1
