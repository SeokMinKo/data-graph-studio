...
class TestBoxPlotChart:
    ...
    def test_outlier_detection(self, chart):
        ...
        data = pl.DataFrame({
            'Category': ['A'] * 10,
            'Value': [10, 11, 10, 11, 10, 11, 10, 11, 100, -50]  # 100, -50 are outliers
        })
...
class TestViolinPlotChart:
    ...
    def sample_data(self):
        return pl.DataFrame({
            ...
            'Value': list(np.random.normal(10, 2, 100)) + 
                     list(np.concatenate([np.random.normal(8, 1, 50), 
                                          np.random.normal(14, 1, 50)]))  # Bimodal
        })
    ...
    def test_include_box_stats(self, chart, sample_data):
        """Box plot 통계 포함"""
        density = chart.calculate_density(sample_data, 'Category', 'Value', include_box=True)
...
class TestWaterfallChart:
    ...
    def sample_data(self):
        return pl.DataFrame({
            ...
            'Type': ['start', 'increase', 'decrease', 'decrease', 'decrease', 'total']
        })
    ...
    def test_calculate_waterfall(self, chart, sample_data):
        """Waterfall 데이터 계산"""
        result = chart.calculate_waterfall(
            sample_data, 'Category', 'Value', 'Type'
        )
    ...
    def test_auto_type_detection(self, chart):
        ...
        result = chart.calculate_waterfall(
            data, 'Category', 'Value', type_col=None
        )
