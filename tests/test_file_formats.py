...
class TestTextLoading:
    ...
    def test_load_space_delimited(self):
        ...
        if path.exists():
            success = self.engine.load_file(
                str(path), delimiter_type=DelimiterType.SPACE
            )
            assert success
            assert self.engine.row_count == 3
    ...
    def test_load_semicolon_delimited(self):
        ...
        if path.exists():
            success = self.engine.load_file(
                str(path), delimiter_type=DelimiterType.SEMICOLON
            )
            assert success
            assert self.engine.row_count == 3
    ...
    def test_load_pipe_delimited(self):
        ...
        if path.exists():
            success = self.engine.load_file(
                str(path), delimiter_type=DelimiterType.PIPE
            )
            assert success
            assert self.engine.row_count == 3
...
class TestAdvancedOptions:
    ...
    def test_skip_rows(self):
        ...
        if path.exists():
            success = self.engine.load_file(
                str(path), skip_rows=2, delimiter_type=DelimiterType.COMMA
            )
            assert success
            assert self.engine.row_count == 3
            assert "name" in self.engine.columns
    ...
    def test_comment_char(self):
        ...
        if path.exists():
            success = self.engine.load_file(
                str(path), comment_char="#", delimiter_type=DelimiterType.COMMA
            )
            assert success
            # 주석 제외하고 3개 데이터 행
            assert self.engine.row_count == 3
...
class TestAutoDelimiter:
    ...
    def test_auto_detect_comma(self):
        ...
        if path.exists():
            success = self.engine.load_file(
                str(path), delimiter_type=DelimiterType.AUTO
            )
            assert success
            assert self.engine.row_count == 3
...
class TestDataTypes:
    ...
    def test_numeric_conversion(self):
        ...
        try:
            success = self.engine.load_file(
                tmp_path, file_type=FileType.TXT, delimiter_type=DelimiterType.SPACE
            )
            assert success
            ...
            assert (
                "Int" in dtypes.get("id", "") or "int" in dtypes.get("id", "").lower()
            )
        finally:
            os.unlink(tmp_path)
...
class TestETLFiles:
    ...
    def test_binary_vs_text_detection(self):
        ...
        try:
            ...
            self.engine.clear()
            success_binary = self.engine.load_file(
                tmp_path_binary, file_type=FileType.ETL
            )
...
class TestEdgeCases:
    ...
    def test_inconsistent_columns(self):
        ...
        try:
            success = self.engine.load_file(
                tmp_path, file_type=FileType.TXT, delimiter_type=DelimiterType.COMMA
            )
            assert success
