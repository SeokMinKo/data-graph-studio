...
class TestOverExpression:
    ...
    def test_percent_of_total(self, parser, sample_data):
        """전체 대비 백분율"""
        result = parser.evaluate(
            "[sales] / Sum([sales]) OVER (AllPrevious([category])) * 100",
            sample_data
        )
    ...
    def test_running_sum(self, parser, sample_data):
        """누적 합계"""
        result = parser.evaluate(
            "Sum([sales]) OVER (AllPrevious([category]))",
            sample_data
        )
...
class TestExpressionValidator:
    ...
    def test_invalid_column(self, validator):
        ...
        assert len(result.errors) > 0
        assert "quantity" in result.errors[0].lower() or "price" in result.errors[0].lower()
