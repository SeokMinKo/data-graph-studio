...
# UI imports
from data_graph_studio.ui.dashboard import DashboardItem, DashboardManager, GridPosition
...
def sample_sales_data():
    ...
    dates = [
        date(2024, np.random.randint(1, 13), np.random.randint(1, 28)) for _ in range(n)
    ]
...
class TestCalculatedFieldsWithFiltering:
    ...
    def test_filter_by_calculated_column(
        self, sample_sales_data, expression_engine, filtering_manager
    ):
        """계산된 컬럼으로 필터링"""
        # 1. 계산 필드 추가: Total = sales * quantity
        df = expression_engine.add_column(
            sample_sales_data, "Total", "sales * quantity"
        )
    ...
    def test_filter_by_profit_margin(
        self, sample_sales_data, expression_engine, filtering_manager
    ):
        """이익률 계산 후 필터링"""
        # 1. 여러 계산 필드 추가
        ...
        assert all(
            col in df.columns for col in ["Revenue", "Cost", "Profit", "ProfitMargin"]
        )
    ...
    def test_calculated_discount_filter(
        self, sample_sales_data, expression_engine, filtering_manager
    ):
        """할인 적용 금액 계산 후 필터링"""
        # 1. 할인 적용 금액 계산
        df = expression_engine.add_column(
            sample_sales_data, "DiscountedSales", "sales * (1 - discount)"
        )

        # 2. 원래 가격 대비 할인 금액
        df = expression_engine.add_column(
            df, "DiscountAmount", "sales - DiscountedSales"
        )
    ...
    def test_multiple_calculated_filters(
        self, sample_sales_data, expression_engine, filtering_manager
    ):
        """여러 계산 필드에 대한 복합 필터링"""
        # 1. 계산 필드들 추가
        ...
        # 2. 여러 필터 적용
        filtering_manager.add_filter(
            "Page", "TotalSales", FilterOperator.GREATER_THAN, 50000
        )
        filtering_manager.add_filter(
            "Page", "UnitProfit", FilterOperator.GREATER_THAN, 0
        )
        filtering_manager.add_filter(
            "Page", "region", FilterOperator.IN_LIST, ["North", "East"]
        )
...
class TestFilteringWithTrendLine:
    ...
    def test_trendline_on_filtered_data(
        self, sample_sales_data, filtering_manager, curve_fitter
    ):
        """필터된 데이터에 추세선 적용"""
        # 1. 필터 적용: North 지역만
    ...
    def test_polynomial_trendline_filtered(
        self, sample_sales_data, filtering_manager, curve_fitter
    ):
        """필터된 데이터에 다항식 추세선"""
        # 1. 특정 카테고리만 필터
        filtering_manager.add_filter(
            "Page", "category", FilterOperator.EQUALS, "Electronics"
        )
        filtered = filtering_manager.apply_filters("Page", sample_sales_data)
    ...
    def test_forecast_filtered_data(
        self, sample_sales_data, filtering_manager, curve_fitter
    ):
        """필터된 데이터에서 예측 (미래 예측)"""
        # 1. 날짜 순 정렬 후 필터
    ...
    def test_trendline_multiple_groups(
        self, sample_sales_data, filtering_manager, curve_fitter
    ):
        """여러 그룹별 추세선 (트렐리스 스타일)"""
        results = {}
        ...
        for region in ["North", "South", "East", "West"]:
            ...
            filtering_manager.add_filter(
                "Page", "region", FilterOperator.EQUALS, region
            )
            filtered = filtering_manager.apply_filters("Page", sample_sales_data)
...
class TestTrellisWithDashboard:
    ...
    def test_trellis_chart_in_dashboard(
        self, sample_sales_data, trellis_calculator, dashboard_manager
    ):
        """트렐리스 차트를 대시보드에 배치"""
        # 1. 트렐리스 레이아웃 계산
    ...
    def test_multiple_trellis_in_dashboard(
        self, sample_sales_data, trellis_calculator, dashboard_manager
    ):
    ...
    def test_trellis_with_shared_filter(
        self, sample_sales_data, trellis_calculator, dashboard_manager
    ):
        """대시보드 공유 필터와 트렐리스"""
        dashboard = dashboard_manager.create_dashboard(
            "Filtered Trellis", rows=3, cols=3
        )
        ...
        # 공유 필터 추가: 특정 카테고리만
        dashboard.layout.add_shared_filter(
...
class TestCalculatedFieldsWithReport:
    ...
    def test_report_with_calculated_statistics(
        self, sample_sales_data, expression_engine
    ):
        """계산 필드 통계를 리포트에 포함"""
        # 1. 계산 필드 추가
        df = sample_sales_data.clone()
    ...
    def test_html_report_with_calculations(
        self, sample_sales_data, expression_engine, tmp_path
    ):
        ...
        report_data = ReportData(
            metadata=ReportMetadata(
                title="Sales Analysis Report", subtitle="With Calculated Fields"
            ),
    ...
    def test_report_with_derived_columns(self, sample_sales_data, expression_engine):
        ...
        df = expression_engine.add_column(
            df, "DiscountAmount", "GrossRevenue * discount"
        )
        df = expression_engine.add_column(
            df, "NetRevenue", "GrossRevenue - DiscountAmount"
        )
...
class TestFullPipeline:
    ...
    def test_complete_analysis_pipeline(
        ...
        df = expression_engine.add_column(
            df, "ProfitMargin", "TotalProfit / TotalRevenue"
        )

        assert all(
            col in df.columns
            for col in ["TotalRevenue", "UnitProfit", "TotalProfit", "ProfitMargin"]
        )

        # ===== STEP 3: 필터링 =====
        filtering_manager.add_filter(
            "Page", "TotalProfit", FilterOperator.GREATER_THAN, 0
        )
        filtering_manager.add_filter(
            "Page", "category", FilterOperator.IN_LIST, ["Electronics", "Clothing"]
        )
        ...
        # ===== STEP 6: 대시보드 =====
        dashboard = dashboard_manager.create_dashboard(
            "Full Analysis Dashboard", rows=4, cols=4
        )

        # 메인 트렐리스 차트
        dashboard.layout.add_item(
            DashboardItem(
                id="main_trellis",
        ...
        # ===== STEP 7: 리포트 =====
        # 통계 수집
        stats = collect_statistics_from_dataframe(
            filtered_df, "filtered_sales", "Filtered Sales"
        )
        ...
        report_data = ReportData(
            ...
            datasets=[
                DatasetSummary.from_dataframe(
                    filtered_df, "main", "Filtered Sales Data"
                )
            ],
            statistics={"main": stats},
            key_findings=[
    ...
    def test_pipeline_with_empty_filter_result(
        self, sample_csv_file, expression_engine, filtering_manager
    ):
        """빈 필터 결과 처리"""
        # 데이터 로드
        engine = DataEngine()
        ...
        # 불가능한 필터 (Total > 무한대)
        filtering_manager.add_filter(
            "Page", "Total", FilterOperator.GREATER_THAN, float("inf")
        )
        filtered = filtering_manager.apply_filters("Page", df)
    ...
    def test_pipeline_with_large_data(
        self, expression_engine, filtering_manager, curve_fitter
    ):
        """대용량 데이터 파이프라인 테스트"""
        # 대용량 데이터 생성
        np.random.seed(42)
...
class TestEdgeCases:
    ...
    def test_null_handling_in_pipeline(self, expression_engine, filtering_manager):
        """NULL 값 처리"""
        df = pl.DataFrame({"a": [1, None, 3, None, 5], "b": [10, 20, None, 40, 50]})
    ...
    def test_empty_dataframe_handling(self, expression_engine, trellis_calculator):
        """빈 DataFrame 처리"""
        empty_df = pl.DataFrame({"a": [], "b": [], "category": []})
        ...
        minimal_df = pl.DataFrame({"a": [1, 2], "b": [10, 20], "category": ["A", "B"]})
    ...
    def test_special_characters_in_columns(self, expression_engine, filtering_manager):
        """특수 문자가 포함된 컬럼명 처리"""
        df = pl.DataFrame({"Sales_2024": [100, 200, 300], "Cost": [50, 100, 150]})
...
class TestPerformance:
    ...
    def test_large_filter_chain(self, filtering_manager):
        ...
        filtering_manager.add_filter(
            "Page", "category", FilterOperator.IN_LIST, ["A", "B"]
        )
