...
def test_windows_frozen_dll_recovery_guide():
    exc = ImportError("DLL load failed while importing PySide6: 지정된 모듈을 찾을 수 없습니다")
...
def test_name_error_recovery_guide():
    steps = main._startup_recovery_guide(NameError("name 'foo' is not defined"), is_frozen=False, platform_name="linux")
...
def test_format_startup_failure_contains_recovery_header():
    msg = main._format_startup_failure("IMPORT ERROR", ModuleNotFoundError("No module named 'x'"))
