...
def test_to_app_state_applies_setting(state):
    ...
    assert state._value_columns == [{"name": "v1"}]
...
def test_round_trip_preserves_data(state):
    ...
    assert new_state._group_columns == [{"name": "g1"}, {"name": "g2"}]
    assert new_state._value_columns == [{"name": "v1"}, {"name": "v2"}]
