...
class TestPollingInterval:
    ...
    def test_set_interval_updates_timer(
        self, mock_fs, mock_timer_factory, csv_data_5rows
    ):
        """폴링 간격 변경 시 타이머도 업데이트"""
        from data_graph_studio.core.file_watcher import FileWatcher

        fw = FileWatcher(
            fs=mock_fs, timer_factory=mock_timer_factory, poll_interval_ms=1000
        )
    ...
    def test_custom_initial_interval(self, mock_fs, mock_timer_factory):
        ...
        fw = FileWatcher(
            fs=mock_fs, timer_factory=mock_timer_factory, poll_interval_ms=5000
        )
...
class TestTailMode:
    ...
    def test_tail_detects_new_rows(
        self, watcher, mock_fs, mock_timer_factory, csv_data_5rows, csv_data_7rows
    ):
        ...
        # Signal 수신을 위한 스파이
        appended_signals = []
        watcher.rows_appended.connect(
            lambda path, count: appended_signals.append((path, count))
        )
    ...
    def test_tail_does_not_emit_file_changed(
        self, watcher, mock_fs, mock_timer_factory, csv_data_5rows, csv_data_7rows
    ):
    ...
    def test_tail_column_mismatch_fallback_to_reload(
        self, watcher, mock_fs, mock_timer_factory, csv_data_5rows
    ):
...
class TestReloadMode:
    ...
    def test_reload_detects_file_change(
        self, watcher, mock_fs, mock_timer_factory, csv_data_5rows
    ):
    ...
    def test_reload_no_change_no_signal(
        self, watcher, mock_fs, mock_timer_factory, csv_data_5rows
    ):
    ...
    def test_reload_mtime_change_only(
        self, watcher, mock_fs, mock_timer_factory, csv_data_5rows
    ):
        ...
        # mtime만 변경, size는 동일
        mock_fs.stats["/data.csv"] = FakeStat(
            st_mtime=1002.0, st_size=len(csv_data_5rows)
        )
...
class TestSelfChange:
    ...
    def test_after_end_self_modify_detects_changes(
        self, watcher, mock_fs, mock_timer_factory, csv_data_5rows
    ):
...
class TestDebounce:
    ...
    def test_multiple_changes_merged(
        self, watcher, mock_fs, mock_timer_factory, csv_data_5rows
    ):
        """연속 변경 이벤트가 하나로 병합"""
        mock_fs.add_file("/data.csv", csv_data_5rows, mtime=1000.0)
    ...
    def test_no_debounce_after_interval(
        self, watcher, mock_fs, mock_timer_factory, csv_data_5rows
    ):
...
class TestFileDeletion:
    ...
    def test_file_deleted_stops_watching(
        self, watcher, mock_fs, mock_timer_factory, csv_data_5rows
    ):
    ...
    def test_permission_denied_emits_file_deleted(
        self, watcher, mock_fs, mock_timer_factory, csv_data_5rows
    ):
...
class TestIntervalClamping:
    ...
    def test_initial_below_minimum_clamped(self, mock_fs, mock_timer_factory):
        ...
        fw = FileWatcher(
            fs=mock_fs, timer_factory=mock_timer_factory, poll_interval_ms=100
        )
    ...
    def test_initial_above_maximum_clamped(self, mock_fs, mock_timer_factory):
        ...
        fw = FileWatcher(
            fs=mock_fs, timer_factory=mock_timer_factory, poll_interval_ms=100000
        )
